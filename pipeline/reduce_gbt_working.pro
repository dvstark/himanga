...
pro reduce_gbt,galaxy,reducer=reducer,overwrite=overwrite,scandb=scandb
  ...
  if 1-keyword_set(scandb) then scandb='/users/dstark/17AGBT012/database/stable/mangahi_scandb.fits'
...
;fix this so we can have bad input and not crash
     ...
     case answer of
        '0':begin
           ...
           print,'loading scans'
           fetch_data,galaxy,scandb,scaninf=scaninfo;'/users/dstark/17AGBT012/database/stable/mangahi_scandb.fits',scaninfo=scaninfo
           unzoom
...
;              fetch_data,galaxy,'/users/dstark/17AGBT012/database/stable/mangahi_scandb.fits',scaninfo=scaninfo
              fetch_data,galaxy,scandb,scaninfo=scaninfo
              ;update scan info  
              par.obsinfo.tsys = !g.s[0].tsys
              par.obsinfo.tint = !g.s[0].exposure
              save_ind = 0
     ...
     if answer ne 'q' then setxunit,'km/s'
